...
void changePointers(int *valueReturned, int *expected, int *var1, int *var2);
...
//after the function completes it should have swapped expected with new value
	...
	pointa = &valueReturned;
	pointb = &expected;
	pointc = &x;
	pointd = &y;

	
	changePointers(pointa, pointb, pointc, pointd);
